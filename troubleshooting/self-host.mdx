...
description: 'Common problems specific to running SocialSyncs yourself'
---

## `pnpm` heap out of memory during install

The SocialSyncs monorepo is large enough that the default Node heap can
...
If `STORAGE_PROVIDER=local`, point `UPLOAD_DIRECTORY` at the path SocialSyncs
...
## Can I deploy SocialSyncs on Vercel?

**Frontend:** yes — it's a standard Next.js app and deploys cleanly to
Vercel.

**Backend:** no. The SocialSyncs backend is a NestJS server that also embeds
...
By default, SocialSyncs uses Resend (`EMAIL_PROVIDER=resend`) and only
...
out — but SocialSyncs auto-activates users in that mode, so signup still
works.

## Temporal isn't reachable

Since v2.12.0, SocialSyncs schedules posts through Temporal. The backend
