...
The SocialSyncs backend accepts JSON bodies up to **50 MB** on the post-creation
...
PDFs are **not** accepted by the public API. The only PDF flow SocialSyncs
...
`/upload-from-url` proxies the source URL through the SocialSyncs backend.
If the source server is slow, unreachable, blocks SocialSyncs's user agent,
...
SocialSyncs inspects uploaded videos for duration, dimensions, and codec
...
You hit Cancel on a file that was already being uploaded. SocialSyncs's
