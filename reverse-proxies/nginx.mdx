---
title: Nginx
description: 'Configure Nginx as a reverse proxy for SocialSyncs'
---
...
server {
    ...
    # Reverse Proxy to SocialSyncs application
    location / {
...
server {
    ...
    # Proxy requests to SocialSyncs running on port 5000
    location / {
        proxy_pass http://localhost:5000;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
...
```
  ...
  <Step title="Done!">
    Your Nginx reverse proxy is now running, securely serving your SocialSyncs instance at your subdomain.domain.tld. 🎉
  </Step>
</Steps>
