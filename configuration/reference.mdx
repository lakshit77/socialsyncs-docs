...
description: 'Environment variables reference for SocialSyncs'
---

SocialSyncs is configured entirely through environment variables. Any change requires
...
This page documents every variable SocialSyncs reads, grouped by purpose.

<Info>
Variables marked **Required** are validated on boot — SocialSyncs will fail to start
...
The URL the **browser** uses to reach the SocialSyncs frontend. Used as the OAuth redirect base and for email links.

```
FRONTEND_URL="https://socialsyncs.example.com"
```

### `NEXT_PUBLIC_BACKEND_URL` <sup>Required</sup>

The URL the **browser** uses to reach the SocialSyncs backend.

```
NEXT_PUBLIC_BACKEND_URL="https://api.socialsyncs.example.com"
```
...
Per-hour limit on the public-API create-post endpoint. Defaults to `90`. SocialSyncs Cloud uses `100`. Channel and post quotas are tiered separately by plan.
...
Switches the frontend between routes available to the open-source build (`/launches`) and the hosted build (`/analytics`). Set to `"true"` on self-host. Leave unset on SocialSyncs Cloud.
...
Since v2.12.0, SocialSyncs uses Temporal for scheduled posts and background workflows.
...
If a SocialSyncs user configures short-link replacement, SocialSyncs proxies link
