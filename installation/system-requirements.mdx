...
description: 'Hardware, services, ports, and network requirements for self-hosting SocialSyncs'
---
...
<Card title="socialsyncs-docker-compose" icon="docker" href="https://github.com/gitroomhq/postiz-docker-compose">
  Pre-wired SocialSyncs + Postgres + Redis + Temporal. The fastest way to a working install.
</Card>
...
SocialSyncs needs four external services:
...
externally, point SocialSyncs at them via `DATABASE_URL`, `REDIS_URL`, and
`TEMPORAL_ADDRESS`.
...
| Service | Port | When |
|---|---|---|
| SocialSyncs container (bundled FE + BE) | `5000` | Official Docker image (`ghcr.io/gitroomhq/postiz-app`) — exposed for you to map. The bundled compose maps host `4007:5000`. |
...
SocialSyncs initiates outbound HTTPS to every social provider you connect.
