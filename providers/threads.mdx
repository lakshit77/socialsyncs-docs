...
<Steps>
  ...
  <Step title="Set threads API settings">
    - Click the "Settings" tab. Copy your "Threads App ID", and set the socialsyncs environment variables `THREADS_APP_ID` to this value.
    - Click the "Show" button next to the Threads App Secret, and set the socialsyncs environment variable `THREADS_APP_SECRET` to this value. Note that the next box is quite small, make sure you scroll across the copy the full value. It should be 32 characters long.

    <Snippet file="oauth2redirect.mdx" />

    **Your Threads OAuth2 Redirect URI:**
    - Production: `https://your-socialsyncs-domain.com/integrations/social/threads`
    ...
    You can use the same value for the **Uninstall Callback URL** and **Delete Callback URL**, but note that SocialSyncs does not support either at this time. The form will not save unless you enter something.
    </Note>
  ...
  <Step title="Restart SocialSyncs">
    Stop SocialSyncs if it is running, and then start it again to pick up the new environment variables.

    You should not try to add a Threads account to SocialSyncs at this time.
  </Step>
  ...
  <Step title="Good luck!">
    At this stage things should be working correctly, try a test post from SocialSyncs to confirm. 
