...
TikTok will also not allow http:// for your app redirect URI, so you will need to be accessing SocialSyncs from HTTPS.
</Note>
...
In practice this means: before your TikTok developer app passes audit, the `privacy_level` setting for posts is forced to **SELF_ONLY** (private) regardless of what you choose in SocialSyncs. Once your app is audited, all privacy levels become available.
</Warning>
...
<Steps>
  <Step title="Create your app">
    ...
    * **App Name:** `MySocialSyncs`
    * **Redirect URI:** (see below)

    <Snippet file="oauth2redirect.mdx" />

    **Your TikTok OAuth2 Redirect URI:**
    - Production: `https://your-socialsyncs-domain.com/integrations/social/tiktok`
...
```
  ...
  <Step title="Add a TikTok channel in the SocialSyncs web interface">
    Go to the SocialSyncs web interface, and click on the "Add Channel" button. Select "TikTok" from the list of available channels. You should be redirected to TikTok to authorize the application.
  </Step>
</Steps>
