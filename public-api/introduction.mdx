---
title: API Overview
description: 'Getting started with the SocialSyncs Public API'
---
...
<CardGroup cols={2}>
  <Card title="NodeJS SDK" icon="node-js" href="https://www.npmjs.com/package/@postiz/node">
    Official SocialSyncs NodeJS SDK
  </Card>
  <Card title="n8n Node" icon="n" href="https://www.npmjs.com/package/n8n-nodes-postiz">
    Custom n8n node for SocialSyncs
  </Card>
</CardGroup>

## Authentication

There are two ways to authenticate with the SocialSyncs API:
...
```bash
curl -H "Authorization: your-api-key" https://app.socialsyncs.co/api/public/v1/integrations
```

### OAuth2 Token

If you're building an app for other SocialSyncs users, use [OAuth2 Authentication](/public-api/oauth) to get tokens that act on behalf of users. OAuth tokens start with `pos_` and are used the same way:

```bash
curl -H "Authorization: pos_your-oauth-token" https://app.socialsyncs.co/api/public/v1/integrations
```
...
|-------------|----------|
| SocialSyncs Cloud | `https://app.socialsyncs.co/api/public/v1` |
...
<Note>
The SocialSyncs UI uses the term **channel**, while the API uses **integration**. They refer to the same thing—a connected social media account.
</Note>

## Generate Output

The easiest way to generate your post payloads is by using this wizard.
It's the same wizard to schedule posts in the SocialSyncs app, however instead of scheduling posts, it generates the JSON payload for you to use in your API requests.

- For cloud, make sure you are logged in.
- For local, make sure your SocialSyncs server is running and your are logged in.

<Tabs>
  <Tab title="Cloud">
      <iframe src="https://app.socialsyncs.co/modal/dark/all" style={{zoom: 0.5, height: 1000, width: '100%'}} />
      <a href="https://app.socialsyncs.co/modal/dark/all" target="_blank" rel="noopener noreferrer" style={{display: 'block', marginTop: '10px', textAlign: 'center'}}>
         Open in full screen
      </a>
  </Tab>
...
<Card title="View Provider Settings Reference" icon="code" href="/public-api/providers/linkedin">
...
# Response: { "id": "img-123", "path": "https://uploads.socialsyncs.co/photo.jpg", ... }
